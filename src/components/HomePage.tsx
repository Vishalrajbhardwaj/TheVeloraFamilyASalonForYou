...
import { Sparkles, MapPin, Clock, Phone, Award, Users, Heart, Scissors, Star, Gift, TrendingUp, Instagram, Facebook, Youtube } from 'lucide-react';
...
const SocialButton = ({ href, bg, children, aria, mobile }: { href: string; bg: string; children: React.ReactNode; aria: string; mobile?: boolean }) => (
  <a
    href={href}
    target="_blank"
    rel="noreferrer"
    aria-label={aria}
    className={mobile ? "flex-1 flex justify-center" : "mb-3"}
  >
    <div
      className={`${mobile ? 'w-10 h-10' : 'w-12 h-12'} flex items-center justify-center rounded-full text-white shadow-md ${bg} transform-gpu transition-transform duration-300 ${mobile ? 'hover:scale-110' : 'hover:-translate-x-2 hover:scale-105'} hover:shadow-2xl social-3d`}
    >
